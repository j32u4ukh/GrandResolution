...
def transform_images_temp(directory, output_directory, scaling_factor=2, max_nb_images=-1, true_upscale=False,
    ...
    # 若 output_directory X 不存在則建立
    if not os.path.exists(output_directory + "X/"):
        os.makedirs(output_directory + "X/")

    # 若 output_directory Y 不存在則建立
    if not os.path.exists(output_directory + "y/"):
        os.makedirs(output_directory + "y/")
    ...
    if max_nb_images != -1:
        ...
        assert max_nb_images <= nb_images, "Max number of images must be less than number of images in path"
        print("Transforming {} images.".format(nb_images))
    ...
    print("Images transformed. Saved at directory : {}".format(output_directory))
...
if __name__ == "__main__":
    ...
    # build /train/X, /train/y
    transform_images_temp(input_path,
                          output_path,
                          scaling_factor=scaling_factor,
                          max_nb_images=-1,
                          true_upscale=true_upscale)

    # transform_images_temp(validation_set5_path,
    #                       validation_output_path,
    #                       scaling_factor=scaling_factor,
    #                       max_nb_images=-1,
    #                       true_upscale=true_upscale)

    # transform_images_temp(validation_set14_path,
    #                       validation_output_path,
    #                       scaling_factor=scaling_factor,
    #                       max_nb_images=-1,
    #                       true_upscale=true_upscale)
