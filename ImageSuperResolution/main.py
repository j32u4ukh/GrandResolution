...
if model_type not in ["sr", "esr", "dsr", "ddsr", "rnsr", "distilled_rnsr"]:
...
with tf.device('/CPU:0'):
    ...
    for p in path:
        if model_type == "sr":
            model = tf_models.ImageSuperResolutionModel(scale_factor)
        elif model_type == "esr":
            model = tf_models.ExpantionSuperResolution(scale_factor)
        elif model_type == "dsr":
            model = tf_models.DenoisingAutoEncoderSR(scale_factor)
        elif model_type == "ddsr":
            model = tf_models.DeepDenoiseSR(scale_factor)
        elif model_type == "rnsr":
            model = tf_models.ResNetSR(scale_factor)
        elif model_type == "distilled_rnsr":
            model = tf_models.DistilledResNetSR(scale_factor)
        else:
            model = tf_models.DistilledResNetSR(scale_factor)
